...
logger = logging.getLogger("pptxforgekit.cli")

# Ensure stdout handles Unicode on Windows terminals (cp949, cp932, etc.)
...
def _model_help() -> str:
    from pptxforgekit.llm.registry import DEFAULT_MODELS, KNOWN_MODELS
...
def _load_provider(llm_provider: str | None, llm_model: str | None) -> LLMProvider | None:
    ...
    try:
        ...
        try:
            ...
            pass
        from pptxforgekit.llm.registry import build_provider
...
@click.group()
@click.version_option(__version__, prog_name="pptxforgekit")
def main() -> None:
...
def init_theme(output: str) -> None:
    ...
    from pptxforgekit.theme.loader import default_theme_yaml
...
def llm_info() -> None:
    ...
    install_extras = {
        "claude": "pip install 'pptxforgekit[llm-claude]'",
        "gpt": "pip install 'pptxforgekit[llm-openai]'",
        "gemini": "pip install 'pptxforgekit[llm-gemini]'",
        "ollama": "pip install 'pptxforgekit[llm-ollama]'",
    }
    ...
    click.echo("  pptxforgekit analyze ./docs -o analysis.json --llm claude")
    click.echo("  pptxforgekit analyze ./docs -o analysis.json --llm gpt --model gpt-4o-mini")
    click.echo("  pptxforgekit plan analysis.json -o outline.json --llm gemini")
    click.echo("  pptxforgekit build-schema outline.json --theme theme.yaml -o slides.json --llm ollama --model mistral")
