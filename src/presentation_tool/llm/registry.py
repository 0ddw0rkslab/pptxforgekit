...
def build_provider(provider: str, model: str | None = None, **kwargs) -> LLMProvider:
    ...
    if provider == "claude":
        from presentation_tool.llm.providers.claude import ClaudeProvider
        return ClaudeProvider(model=resolved_model, **kwargs)

    if provider == "gpt":
        from presentation_tool.llm.providers.openai_p import OpenAIProvider
        return OpenAIProvider(model=resolved_model, **kwargs)

    if provider == "gemini":
        from presentation_tool.llm.providers.gemini import GeminiProvider
        return GeminiProvider(model=resolved_model, **kwargs)

    if provider == "ollama":
        from presentation_tool.llm.providers.ollama import OllamaProvider
